 <!DOCTYPE html>
 <html lang="en">
 <head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Kirana Shop</title>
  <link rel="icon" href="/images/Smartn.jpg" type="image/x-icon">
  <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.1/css/all.min.css">
  <link rel="stylesheet" type="text/css" href="/css/indexstyles.css">
 </head>
 <body>
                                 <!--header-->
    <div class="header">
                                 <!--header-left-->
        <div class="header-left"> 
                        <!--header-left-part-1-->
            <div class="logo">
                <img src="/images/smartn.jpg" height="60" >
            </div>
                        <!--header-left-part-2-->
            <div class="menu">
                <ul>
                    <li><a href="#">MENU</a></li>
                    <li><a href="#">About</a></li>
                    <li><a href="#">Products</a></li>
                    <li><a href="contactpage">Contact Us</a></li>
                </ul>
            </div>
        </div>
        
                                 <!--header-right-->
        <div class="header-right">
            <!--header-right-part-1-->
            <form>
                <button><i class="fa-solid fa-magnifying-glass"></i></button>
                <input type="text" name="" placeholder="Search for product">
            </form>

            <!--header-right-part-2-->
            <div class="profile">
                <ul>
                    <li><i class="fa-solid fa-user"></i><b>Profile</b></li>
                    <li><i class="fa-solid fa-heart"></i><b>wishlist</b></li>
                    <li><i class="fa-solid fa-bag-shopping"></i><b>bag</b></li>
                </ul>
            </div>
        </div>
    </div>    <!--header section end-->
    
                   <!-- Main Section -->
  <section class="section1">
    <h2>Welcome to</h2>
    <h1>Kirana Shop</h1>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus lacinia odio vitae vestibulum vestibulum.</p>
    <button><a href="contactpage">Contact Us</a></button>
  </section>
  
 
                <!--section-2-->
    <div class="brandsection">
        <h2>MEDAL WORTHY BRANDS to BAG</h2>
        <div class="brandrow">
            <div class="branditems">
                <img src="/images/product/anis-m-WnVrO-DvxcE-unsplash.jpeg" alt="">
            </div>
            <div class="branditems">
                <img src="/images/product/evan-mcdougall-qnh1odlqOmk-unsplash.jpeg" alt="">
            </div>
            <div class="branditems">
                <img src="/images/product/jordan-nix-CkCUvwMXAac-unsplash.jpeg" alt="">
            </div>
            <div class="branditems">
                <img src="/images/product/nature-zen-3Dn1BZZv3m8-unsplash.jpeg" alt="">
            </div>
            <div class="branditems">
                <img src="/images/product/quokkabottles-kFc1_G1GvKA-unsplash.jpeg" alt="">
            </div>
            <div class="branditems">
                <img src="/images/product/team-fredi-8HRKoay8VJE-unsplash.jpeg" alt="">
            </div>
        </div>
    </div>


 <!--section-3-->
 <div class="brandsection">
    <h2>MEDAL WORTHY BRANDS to BAG</h2>
    <div class="brandrow">
        <div class="branditems">
            <img src="/images/imegica/about-1.jpg" alt="">
        </div>
        <div class="branditems">
            <img src="/images/imegica/about-2.jpg" alt="">
        </div>
        <div class="branditems">
            <img src="/images/imegica/carousel-1.jpg" alt="">
        </div>
        <div class="branditems">
            <img src="/images/imegica/classes-4.jpg" alt="">
        </div>
        <div class="branditems">
            <img src="/images/imegica/classes-5.jpg" alt="">
        </div>
         
    </div>
</div>

     <!-- Footer -->
  <footer>
    <div class="footer-container">
      <!-- About Shop -->
      <div class="footer-section">
        <h3>About Shop</h3>
        <p>Address</p>
        <p>+01 1234567890</p>
        <p>demo@gmail.com</p>
      </div>

      <!-- Informations -->
      <div class="footer-section">
        <h3>Informations</h3>
        <p>ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt</p>
      </div>

      <!-- Instagram -->
      <div class="footer-section">
        <h3>Instagram</h3>
        <div class="instagram-grid">
          <div class="instagram-item"></div>
          <div class="instagram-item"></div>
          <div class="instagram-item"></div>
          <div class="instagram-item"></div>
          <div class="instagram-item"></div>
          <div class="instagram-item"></div>
        </div>
      </div>

      <!-- Newsletter -->
      <div class="footer-section">
        <h3>Newsletter</h3>
        <form>
          <input type="email" placeholder="Enter your email" class="newsletter-input">
          <button type="submit" class="subscribe-button">SUBSCRIBE</button>
        </form>
        <br>
        <div class="social-links">
          
          <a href="#"><img src="/images/fb.png" alt="Facebook" class="icon"></a>
          <a href="#"><img src="/images/twitter.png" alt="Twitter" class="icon"></a>
          <a href="#"><img src="/images/linkedin.png" alt="LinkedIn" class="icon"></a>
          <a href="#"><img src="/images/youtube.png" alt="YouTube" class="icon"></a>
          
        </div>
      </div>
    </div>
    <div class="footer-bottom">
      _____________________________________________________________________________________
      <br>
      © 2020 All Rights Reserved By Free Html Templates
    </div>
  </footer>

 </body>
 </html>